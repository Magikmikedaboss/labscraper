...
def check_longevity_compounds():
    ...
    import re
    compounds_text = Path('seeds/compounds.txt').read_text(encoding='utf-8').lower()
    # Tokenize on non-word characters and normalize
    # Allow trailing '+' in tokens (e.g., 'nad+')
    parsed_compounds_set = set(re.findall(r'\b\w[\w+\- ]*\w\+?\b|\b\w\+?\b', compounds_text))
    ...
    lines = Path('seeds/compounds.txt').read_text(encoding='utf-8').split('\n')
