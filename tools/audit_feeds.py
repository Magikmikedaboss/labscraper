...
def _rss_pdf_urls(entry: dict[str, Any]) -> list[str]:
    pdf_urls = list(dict.fromkeys(extract_pdf_links(entry)))
    ...
    for link in entry.get("links", []):
        ...
        if href and ("application/pdf" in str(link.get("type", "")).lower() or str(link.get("title", "")).lower() == "pdf"):
...
def _summarize_pdf_list_feed(feed: dict[str, Any], *, max_pdf_attempts: int | None = None) -> dict[str, Any]:
    ...
    for pdf_url in pdf_urls:
        ...
        if max_pdf_attempts is not None and len(processed_urls) > max_pdf_attempts:
            ...
            continue
        downloaded, accepted = _record_pdf_outcome(
            feed=feed,
            pdf_url=pdf_url,
        ...
        pdfs_downloaded += int(downloaded)
        pdfs_accepted += int(accepted)
