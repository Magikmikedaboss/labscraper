...
This step targets the optimized phase-1 scraper entrypoint and expects `input/pdfs/`.
...
This also targets the optimized phase-1 scraper entrypoint. If you need the compatibility fallback, use `utils/scrape_pdfs_phase1.py` instead. If you use the parallel scraper, point it at the same folder layout with `--input-dir input/pdfs` or a matching `input_pdfs/` symlink.
