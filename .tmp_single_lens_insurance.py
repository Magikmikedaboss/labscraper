"""Temporary cache-scan utility for insurance-risk lens extraction.
...
def main(
    ...
    out_dir: Path = Path("tmp/insurance_lens_report"),
    ...
    logger.info(
        "Starting insurance lens cache scan cache_dir=%s max_pdfs=%s source_type=%s",
        cache_dir,
        max_pdfs,
        source_type,
    )
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        for index, (pdf_name, local_rows, local_hits) in enumerate(
            executor.map(partial(_scan_pdf, source_type=source_type), pdf_paths),
            start=1,
        ):
    ...
    csv_path = out_dir / "insurance_lens_cache_hits.csv"
    ...
    summary_path = out_dir / "insurance_lens_cache_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    logger.info("Wrote %s", csv_path)
    logger.info("Wrote %s", summary_path)
    logger.info("insurance_risk: pdf_hits=%s, sentence_hits=%s", summary["pdfs_with_hits"], summary["total_hits"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", default="tmp/insurance_lens_report")
    parser.add_argument("--max-pdfs", type=int, default=None)
    parser.add_argument(
        "--source-type",
        default=None,
        help='Source type passed to construction_insurance_risk_v1.detect; defaults to "news" for cache/rss and "research_paper" otherwise',
    )
