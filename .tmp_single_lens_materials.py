"""Temporary cache-scan utility for materials lens extraction.
...
def main(
    ...
    out_dir: Path = Path("tmp/materials_lens_report"),
    max_pdfs: int | None = None,
    progress_interval: int = 25,
) -> int:
    max_pdfs = _env_limit("LABSCRAPER_MAX_PDFS") if max_pdfs is None else max_pdfs

    _configure_logging(logger, out_dir, "materials_lens_cache_results.log")

    pdf_paths = sorted(cache_dir.glob("*.pdf"))
    rows: list[dict] = []
    pdf_hits: dict[str, int] = {}

    logger.info("Starting materials lens cache scan cache_dir=%s max_pdfs=%s", cache_dir, max_pdfs)
    ...
    summary_path = out_dir / "materials_lens_cache_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    logger.info("Wrote %s", csv_path)
    logger.info("Wrote %s", summary_path)
    logger.info("materials: pdf_hits=%s, sentence_hits=%s", summary["pdfs_with_hits"], summary["total_hits"])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out-dir", default="tmp/materials_lens_report")
