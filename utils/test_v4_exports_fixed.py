"""
Test v4 Professional Exports - Fixed Version
Validates all 4 tweaks are working correctly with proper error handling
"""
...
def test_process_words_demoted():
    ...
    if not context_process_words:
        print("❌ FAIL: No process words found in top entities as context! Test failed.")
        return False  # or: raise AssertionError("No process words found in top entities as context!")
...
def test_confidence_boost():
    ...
    if events and ('confidence_original' not in events[0] or 'confidence_boosted' not in events[0]):
        print(f"❌ FAIL: Missing 'confidence_original' or 'confidence_boosted' column")
        return False    
    ...
    print(f"\n✅ Boost rule: {meta.get('confidence_boost_rule', 'Not specified')}")
...
def test_entity_count_columns():
    ...
    required_cols = ['primary_entity_count', 'context_entity_count', 
                     'entities_primary', 'entities_context', 'all_entities']
    ...
    primary_counts = Counter(safe_int(e['primary_entity_count']) for e in events)
    context_counts = Counter(safe_int(e['context_entity_count']) for e in events)    
...
def run_all_tests():
    ...
    print("\n" + "="*70)
    print("🧪 TESTING v4 PROFESSIONAL EXPORTS - FIXED VERSION")
    print("="*70)
