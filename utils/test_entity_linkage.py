...
def main():
    ...
    with connect_with_foreign_keys(DB_PATH) as con:
        ...
        # Get the true total count of compound→model links
        cur.execute("""
                ...
                WHERE ent1.entity_type = 'compound'
                    AND ent2.entity_type = 'model'
        """)
        compound_model_count = cur.fetchone()[0]

        if compound_model_events:
            print(f"  ✓ Found {compound_model_count} events linking compounds to models\n")
